...
## Status
The bash scripts in `scripts/` below are still the current, working tool for `reclaim` —
nothing about how you use that one has changed. A packaged Python CLI is being built in
...
Arch Linux). `cleanup sort` is also now a real, working ported command (see the callout below for
a behavior change vs. the old script). Keep using `safe-reclaim.sh` until the CLI has ported it too
and this README says otherwise. The CLI now reads (and creates, if absent) an optional config file
at `~/.config/cleanup-tools/config.yaml` for bucket rules, search roots, and master paths — sensible
